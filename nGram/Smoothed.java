...
public class Smoothed {
	...
	public int problem3(String fileName, String secFileName) throws FileNotFoundException, UnsupportedEncodingException {
		double tempNum = 0.0;
		...
		for(String key : count.keySet()) {
			...
			for(String curr : count.get(key).keySet()) {
				//add smoothing at this stage so it is ony done once per
				count.get(key).put(curr, count.get(key).get(curr) + 0.1);
		...
		PrintWriter write = new PrintWriter("smooth_probs.txt", "UTF-8");
		...
		PrintWriter probWriter = new PrintWriter("smoothed_eval.txt", "UTF-8");
